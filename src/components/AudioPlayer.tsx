...
export default function AudioPlayer({ url, isPlayingGlobal, setIsPlayingGlobal }: AudioPlayerProps) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50" style={{ bottom: "calc(1.5rem + env(safe-area-inset-bottom, 0px))", right: "calc(1.5rem + env(safe-area-inset-right, 0px))" }}>
      <motion.button
        onClick={togglePlay}
        className="relative flex h-14 w-14 items-center justify-center rounded-full border border-champagne/40 bg-elegant/90 text-gold shadow-elevated backdrop-blur-md cursor-pointer focus-visible:outline-2 focus-visible:outline-gold"
      ...
      >
        {isPlaying && (
          <>
            <span className="absolute -inset-1 animate-ping rounded-full border border-champagne/30 opacity-75" />
            <span className="absolute -inset-2 animate-pulse rounded-full border border-champagne/15 opacity-40" />
          </>
        )}
